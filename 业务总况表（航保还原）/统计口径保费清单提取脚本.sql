...
insert into lzm_fujian_hangbaohuanyuan_pre
  ...
  where  A.Statdate between date'2015-01-01' and date'2015-12-31'--替换日期
  and    (A.Statflag = 'Y' or A.Statflag is null)
  and    a.classcode not in ('E', '8', '9', 'W', 'V')
  and    A.Riskcode not in ('JAK', 'ECU', 'JHE')
...
insert into lzm_fujian_hangbaohuanyuan_pre
  select a.policyno as 保单号
         ...
         ,a.enddate as 终保日期
         ,a.statdate as 统计日期
         ,nvl(c.mappingcomcode,a.comcode) as 机构代码
         ,a.Handler1Code as 归属业务员代码
         ,a.CoinsFlag as 共保标志
         ,a.currency as 币别
         ,0 as 保额
         ,B.Cnyallocfee * A.Coinsrate / 100 as 保费
  from   cgcmain A
  join   cgcalloc B
  ...
  where  B.statdate between date'2015-01-01' and date'2015-12-31'--替换日期
  and    A.riskcode IN ('JAK', 'JHE')
  and    (A.statflag = 'Y' or A.statflag is null)
  and    B.Cnyallocfee * A.Coinsrate / 100 <> 0
  and    (a.comcode like '3501%' or c.mappingcomcode like '3501%');
commit;
...
insert into lzm_fujian_hangbaohuanyuan_pre
  select a.policyno as 保单号
         ...
         ,a.currency as 币别
         ,0as 保额
         ,B.Cnychgallocfee * A.Coinsrate / 100 as 保费
  from   cgpmain A
  join   cgpalloc B
  on     A.Endorseno = B.Endorseno
  left join
         cgdmccommapping c
  on     a.comcode = c.comcode
  where  B.statdate between date'2015-01-01' and date'2015-12-31'--替换日期
  and    A.riskcode IN ('JAK', 'JHE')
  and    B.Cnychgallocfee * A.Coinsrate / 100 <> 0
  and    (a.comcode like '3501%' or c.mappingcomcode like '3501%');
commit;
